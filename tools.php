...
function get_status($position){
    $file = fopen("/home/factwww/smartfact/data/status.data", "r") or exit("Unable to open file!");
...
function median_voltage(){
    $file = fopen("/home/factwww/smartfact/data/voltage.data", "r") or exit("Unable to open file!");
    for ($x=0; $x<=2; $x++) {
        $temp=fgets($file);
    }
    fclose($file);
